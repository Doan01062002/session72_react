...
//hàm thêm mới user
export const addNewUser:any = createAsyncThunk("users/addUser",
    async (user)=>{
...
//hàm xóa  user
export const deleteUser:any = createAsyncThunk("users/deleteUser",
     (id)=>{
...
export const updateUser:any = createAsyncThunk("users/updateUser",
  async ({ id, name }: { id: number; name: string }) => {
    const response = await axios.patch(`http://localhost:3000/users/${id}`, { name });
    return response.data;
  }
)
...
export const {} = reducerUsers.actions
